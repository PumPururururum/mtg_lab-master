...
class ManaCostStats {
    constructor(deck) {
        this.deck = deck;  // Принимаем колоду в конструктор
    }

    buildStats(element) {
        // Группируем карты по стоимости маны
        const manaMap = this.deck.reduce((acc, card) => {
            const cost = card.cmc || 0;  // Используем свойство cmc для определения стоимости маны
        ...
        // Устанавливаем параметры диаграммы
        const margin = { top: 30, right: 30, bottom: 70, left: 60 };
        const width = 460 - margin.left - margin.right;
        const height = 400 - margin.top - margin.bottom;

        // Очищаем элемент перед построением
        element.innerHTML = '';
